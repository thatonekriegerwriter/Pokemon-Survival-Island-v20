...
Kernel.pbMessage("#{$player.name} misses the #{accessory} #{name} and puts on the #{arr[0][0][0][$PokemonGlobal.playerID]} one instead.")
...
filepath+= $player.isFemale? ? "introGirl" : "introBoy"
...
# Intro Image/Trainercard Image
filepath="Graphics/Pictures/"
helpr=$player.isFemale? ? "introGirl" : "introBoy" #Modify this line if you want more than two characters.
folder=SPRITE_CONVERT_HASH[helpr]
saveBase(filepath+helpr,folder)
# Map Player
