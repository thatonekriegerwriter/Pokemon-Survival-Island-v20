...
class PokemonMailAdapter
  ...
  def getDisplayPrice(item, selling = false)
    return _INTL("Sold out") if $bag.has?(item)
    return _INTL("Sold out") if NoteConfig::NOTES_BACKGROUND.include?(item)
...
class MailMart_Scene
  ...
  def pbRefresh
    if @subscene
      ...
      @sprites["qtywindow"].visible = false #!itemwindow.item.nil?
...
#
#===============================================================================
class MailMartScreen
  def initialize(scene, stock)
    @scene = scene
    @stock = stock
    @adapter = PokemonMailAdapter.new
  end
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if $bag.has?(item)
        pbDisplayPaused(_INTL("Sorry, that item is sold out."))
        next
      end
      if NoteConfig::NOTES_BACKGROUND.include?(item)
        pbDisplayPaused(_INTL("Sorry, that item is sold out."))
        next
      end
...
def pbMailMart(stock, speech = nil, cantsell = false)
  commands = []
  cmdBuy  = -1
  cmdSell = -1
  cmdQuit = -1
  ...
  loop do
    if cmdBuy >= 0 && cmd == cmdBuy
      scene = MailMart_Scene.new
      screen = MailMartScreen.new(scene, stock)
      screen.pbBuyScreen
