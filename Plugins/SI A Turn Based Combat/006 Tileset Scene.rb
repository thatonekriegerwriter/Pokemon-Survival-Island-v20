...
class PokemonTilesetScene
  ...
  def copy_terrain_tag
    tile_id = tile_ID_from_coordinates(@x, @y)
    @copied_terrain_tag = @tileset.terrain_tags[tile_id] || 0
  end

  def paste_terrain_tag
    return if @copied_terrain_tag.nil?

    tile_id = tile_ID_from_coordinates(@x, @y)
    old_tag = @tileset.terrain_tags[tile_id]
    return if old_tag == @copied_terrain_tag

    set_terrain_tag_for_tile_ID(tile_id, @copied_terrain_tag)
    draw_overlay
  end

  def copy_terrain_tags_to_tileset(source_id)
    destination = @tilesets_data[source_id]
    return unless destination


    max_id = [@tileset.terrain_tags.xsize, destination.terrain_tags.xsize].min

    (TILESET_START_ID...max_id).each do |tile_id|
      destination.terrain_tags[tile_id] = @tileset.terrain_tags[tile_id]
   end
    draw_overlay
  end
  ...
  def copy_between_tilesets
  commands = []
  source_ids = []
  (1...@tilesets_data.length).each do |i|
    next if i == @tileset.id
  ...
  ret = pbShowCommands(nil, commands, -1)
  copy_terrain_tags_to_tileset(source_ids[ret]) if ret >= 0
  end
  ...
  def pbStartScene
    ...
    loop do
      ...
      if Input.repeat?(Input::UP)
        ...
        update_cursor_position(0, @visible_height)
	  elsif Input.press?(Input::CTRL) && Input.triggerex?(:C)
         copy_terrain_tag
        ...
        commands = [
          ...
          _INTL("Copy terrain tags to tileset"),
          _INTL("Cancel")
        ]
