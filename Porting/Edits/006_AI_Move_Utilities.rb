class PokeBattle_AI
  ...
  def pbRoughDamage(move,user,target,skill,baseDmg)
    ...
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :GRASS
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=4 && target.pbOwnedByPlayer? && type == :BUG
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=3 && target.pbOwnedByPlayer? && type == :GHOST
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :DRAGON #Boss one
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=2 && target.pbOwnedByPlayer? && type == :ICE
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :GROUND
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :FLYING
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :ELECTRIC
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :WATER
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :STEEL
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :PSYCHIC
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :ROCK
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :POISON
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :FIRE
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :DARK
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :FIGHTING
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :NORMAL
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :CRYSTAL
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=1 && target.pbOwnedByPlayer? && type == :WIND
    multipliers[:base_damage_multiplier] *= 1.4 if $game_variables[255]>=2 && target.pbOwnedByPlayer? && type == :FAIRY
