class PokeBattle_Battler
  ...
  def pbCanInflictStatus?(newStatus,user,showMessages,move=nil,ignoreStatus=false)
    ...
    if self.happiness == 255 && rand(255) >= 240 && self.status = :NONE
      @battle.pbDisplay(_INTL("{1} gathered all its energy to break the status before it started!",pbThis(true))) if showMessages
      return false
    end
