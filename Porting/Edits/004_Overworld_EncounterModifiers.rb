...
if maps.include?($game_map.map_id) && GameData::EncounterType.get($PokemonTemp.encounterType).type == :land  &&($game_variables[4974]=="ADALYNN"||$game_variables[4974]=="Adalynn"||$game_variables[4974]=="adalynn"||$game_variables[4974]=="SNOW"||$game_variables[4974]=="Snow"||$game_variables[4974]=="snow"|| $game_variables[4974]=="POTS"||$game_variables[4974]=="Pots"||$game_variables[4974]=="pots")
  pkmn = e[0]
  if rand(18)==0
  pkmn.species = :VULPIX
  pkmn.level = 80
...
Events.onWildPokemonCreate += proc { |_sender, e|
   ...
   if $game_switches[76]#Earthquakes
     ...
     pokemon.name="Veles"
     pokemon.form=1
    elsif $game_switches[83]#Fire
     pokemon.name="Hestia"
     pokemon.form=1
   elsif $game_switches[90]#Test
     pokemon.name="Recover Test"
     pokemon.learn_move(:RECOVER)
     pokemon.learn_move(:SOFTBOILED)
     pokemon.learn_move(:SLACKOFF)
     pokemon.learn_move(:HEALORDER)
   elsif $game_switches[91]#Test
     pokemon.name="Status Test"
     pokemon.learn_move(:TRIATTACK)
     pokemon.learn_move(:EMBER)
     pokemon.learn_move(:THUNDERSHOCK)
     pokemon.learn_move(:TWINEEDLE)
   end
}
