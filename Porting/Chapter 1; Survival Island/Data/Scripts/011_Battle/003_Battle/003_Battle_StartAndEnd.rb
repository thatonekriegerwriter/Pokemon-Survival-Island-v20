class PokeBattle_Battle
  ...
  def pbStartBattleSendOut(sendOuts)
    ...
    if wildBattle?
      ...
      when 1
        speciesname=Gamedata::Species.get(wildpoke)
        if $game_switches[76]#Earthquakes
           pbDisplayPaused(_INTL("You have awoken the {2}, {1}! Master of earthquakes!",wildpoke.name,speciesname))
          elsif $game_switches[77]#Sea
           pbDisplayPaused(_INTL("You have awoken the {2}, {1}! Lord of the sea!",wildpoke.name,speciesname))
          elsif $game_switches[78]#Storms
           pbDisplayPaused(_INTL("You have awoken the {2}, {1}! Spirit of storms!",wildpoke.name,speciesname))
          elsif $game_switches[79]#Snow
           pbDisplayPaused(_INTL("You have awoken the {2}, {1}! Master of ice and snow!",wildpoke.name,speciesname))
          elsif $game_switches[80]#Forests
           pbDisplayPaused(_INTL("You have awoken the {2}, {1}! Supreme spirit of the forests!",wildpoke.name,speciesname))
         elsif $game_switches[81]#Underworld
           pbDisplayPaused(_INTL("You have awoken the {2}, {1}! Judge of the underworld!",wildpoke.name,speciesname))
         elsif $game_switches[81]#Fire
           pbDisplayPaused(_INTL("You have awoken the {2}, {1}! Searing Faemother of the flames!",wildpoke.name,speciesname))
         else
  ...
  def pbGainMoney
    ...
    if trainerBattle?
      ...
      pbPlayer.money += 0
      moneyGained = 0
      if @field.effects[PBEffects::AmuletCoin]
      pbPlayer.money += tMoney
      moneyGained = pbPlayer.money-oldMoney
      end
      if @field.effects[PBEffects::HappyHour]
      pbPlayer.money += tMoney
      moneyGained = pbPlayer.money-oldMoney
      end
      if moneyGained>0
