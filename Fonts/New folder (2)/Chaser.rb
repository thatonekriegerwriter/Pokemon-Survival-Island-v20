...
class Game_ChaserFactory
  ...
  def each_follower
    $PokemonGlobal.chaser.each_with_index { |follower, i| yield @events[i], follower }
  end
...
module Chaser
  ...
  def add(event_id, name, common_event_id)
    puts "did it"
    $game_temp.chasers.add_follower($game_map.events[event_id], name, common_event_id)
  end

  # @param event [Game_Event] map event to be added as a follower
  def add_event(event)
    $game_temp.chasers.add_follower(event)
  end

  # @param name [String] identifier name of the follower to be removed
  def remove(name)
    $game_temp.chasers.remove_follower_by_name(name)
  end

  # @param event [Game_Event] map event to be removed as a follower
  def remove_event(event)
    $game_temp.chasers.remove_follower_by_event(event)
  end
