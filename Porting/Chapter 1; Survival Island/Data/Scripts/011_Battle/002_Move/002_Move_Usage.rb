    ...
    if damage>=target.hp
      ...
      if nonLethal?(user,target)
        ...
        if target.hasActiveAbility?(:STURDY) && !@battle.moldBreaker
          ...
          damage -= 1
        #edit
#        elsif target.happiness >= 230 && @battle.pbRandom(100)<10
...
#          damage -= 1
  ...
  def pbEffectivenessMessage(user,target,numTargets=1)
    ...
    if Effectiveness.super_effective?(target.damageState.typeMod)
      ...
      elsif user.happiness == 255 && rand(255) >= 240
         @battle.pbDisplay(_INTL("That's it! I knew you could do it!!",user.pbThis(true)))
         @battle.pbDisplay(_INTL("It's super effective!!",user.pbThis(true)))
      else
      ...
      #edit
        ...
        $Trainer.pokedex.register_weakness(species)
      else
