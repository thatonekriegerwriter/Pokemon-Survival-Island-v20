...
class TrainerPC
  ...
  def access
    pbMessage(_INTL("\\se[PC access]You opened up your PC.",$Trainer.name))
    pbTrainerPCMenu
  end
end
...
def pbPokeCenterPC
  pbMessage(_INTL("\\se[Voltorb Flip Tile]{1} logged onto the PC.",$Trainer.name))
  command = 0
  loop do
    commands = PokemonPCList.getCommandList
    command = pbMessage(_INTL("Which part of the PC should be accessed?"),commands,
...
def pbStoragePCMenu
  ...
  loop do
    ...
    case command
    when 0 then pbStoragePCStorage
...
PokemonPCList.registerPC(SaveSystemPC.new)
