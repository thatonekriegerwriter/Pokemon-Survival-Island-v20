class PokeBattle_Battle
  ...
  def pbGainEVsOne(idxParty,defeatedBattler)
    ...
    if pkmn   # Infected or cured
      evYield.each_key { |stat| evYield[stat] *= 5 }
    end
    if pkmn.pokerusStage>=1   # Infected or cured
      evYield.each_key { |stat| evYield[stat] *= 2 }
    end
    if pkmn.happiness>=200   # Infected or cured
      evYield.each_key { |stat| evYield[stat] += 10 }
    end
    if pkmn.sleep>=1 && pkmn.sleep<=20
      evYield.each_key { |stat| evYield[stat] += 4 }
    end
    if pkmn.sleep>20 && pkmn.sleep<=40
      evYield.each_key { |stat| evYield[stat] += 8 }
    end
    if pkmn.sleep>40 && pkmn.sleep<=60
      evYield.each_key { |stat| evYield[stat] += 8 }
    end
    if pkmn.sleep>60 && pkmn.sleep<=80
      evYield.each_key { |stat| evYield[stat] += 5 }
    end
    if pkmn.sleep>80 && pkmn.sleep<=100
      evYield.each_key { |stat| evYield[stat] += 1 }
    end
    # Gain EVs for each stat in turn
    if pkmn.shadowPokemon? && pkmn.saved_ev && !pkmn.loyalty==255
  ...
  def pbGainExpOne(idxParty,defeatedBattler,numPartic,expShare,expAll,showMessages=true)
    ...
    exp =(exp*1.2).floor if pkmn.happiness >= 240 && rand(255)>=240 
    ...
    if showMessages
      ...
      elsif pkmn.happiness>=230 && rand(255)>=240
        pbDisplayPaused(_INTL("{1} got a boosted {2} Exp. Points!",pkmn.name,expGained))
      else
    ...
    # Give Exp
    if pkmn.shadowPokemon? && !pkmn.loyalty==255
      pkmn.exp += expGained
      return
    end
    tempExp1 = pkmn.exp
