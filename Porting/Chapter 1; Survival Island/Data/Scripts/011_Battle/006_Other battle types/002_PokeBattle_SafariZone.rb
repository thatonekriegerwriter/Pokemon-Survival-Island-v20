...
class SafariDataBox < SpriteWrapper
  ...
  def refresh
    ...
    textpos = []
    textpos.push([_INTL("Pokeballs"),30,2,false,base,shadow])
...
class PokeBattle_SafariZone
  ...
  def pbStartBattle
    begin
      ...
      elsif $PokemonBag.pbQuantity(:GREATBALL) > 0
      safariBall = GameData::Item.get(:GREATBALL).id
      elsif $PokemonBag.pbQuantity(:ULTRABALL) > 0
      safariBall = GameData::Item.get(:ULTRABALL).id
      elsif $PokemonBag.pbQuantity(:SUPERBALL) > 0
      ...
      loop do
        ...
        case cmd
        when 0   # Ball
         if $PokemonBag.pbQuantity(safariBall) == 0
        ...
        when 1   # Bait
         if $PokemonBag.pbQuantity(:BAIT) < 1
        ...
        when 3   # Run
         if pbRandom(100)<=25
          ...
          pbDisplayPaused(_INTL("You cannot flee!"))
         elsif pbRandom(100)<=50 && ($Trainer.playerstamina <= 10 || $Trainer.playersleep <= 40)
          ...
          pbDisplayPaused(_INTL("You got away, but tripped on a rock!")) 
              $Trainer.playerhealth -= 10
          @decision = 3
         elsif pbRandom(100)<=50 && catchFactor>=10
          pbSEPlay("Battle flee")
          pbDisplayPaused(_INTL("While you were running away, the POKeMON attacked!")) 
              $Trainer.playerhealth -= 10
         elsif pbRandom(100)<=50 && catchFactor>=10 && ($Trainer.playerstamina <= 10 || $Trainer.playersleep <= 40)
          pbSEPlay("Battle flee")
          pbDisplayPaused(_INTL("While you were running away, the POKeMON attacked!")) 
        ...
        # End of round
        if @decision==0
          if pbRandom(100)<2*escapeFactor
            pbSEPlay("Battle flee")
            ...
            if ($Trainer.playerstamina <= 10 || $Trainer.playersleep <= 40) && pbRandom(100)<=35
              pbDisplay(_INTL("You are too tired to dodge, and  get hit.",pkmn.name))
              $Trainer.playerhealth -= 15
            elsif ($Trainer.playerstamina <= 10 || $Trainer.playersleep <= 40) && pbRandom(100)<=75
            ...
            if ($Trainer.playerstamina <= 10 || $Trainer.playersleep <= 40)
              pbDisplay(_INTL("You are too tired to dodge, and  get hit.",pkmn.name))
              $Trainer.playerhealth -= 15
            elsif ($Trainer.playerstamina <= 10 || $Trainer.playersleep <= 40)
              pbDisplay(_INTL("Your body is lethargic, and you are hit hard.",pkmn.name)) 
              $Trainer.playerhealth -= 30
            elsif pbRandom(100)<=15
              pbDisplay(_INTL("You are primed to dodge, but it moves fast, and hits you.",pkmn.name)) 
