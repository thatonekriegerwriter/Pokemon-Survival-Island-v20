...
ItemHandlers::UseFromBag.add(:BAIT,proc { |item|
  pbMessage(_INTL("You throw the bait on the ground, and soon enough, a POKeMON appeared!"))
  pbEncounter(:Bait)
  next 3
})
...
ItemHandlers::UseInField.add(:CAR,proc { |item|
  if pbBikeCheck
    if $PokemonGlobal.car==true
      pbCommonEvent(75)
    else
      pbCommonEvent(74)
    end
...
ItemHandlers::UseOnPokemon.add(:HEALTHWING,proc { |item,pkmn,scene|
  ...
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}'s HP increased.",pkmn.name))
  pkmn.changeHappiness("wing",pkmn)
  pkmn.changeLoyalty("wing",pkmn)
  next true
})
