...
class MiningGameScene3
  ...
  def pbNoDuplicateItems(newitem)
    return true if newitem==[:STONE,:HARDSTONE,:LIGHTCLAY,:COAL]   # Allow multiple Heart Scales
...
def pbMiningGame3
 ...
 case $Trainer.playerstamina
  when 0
     $Trainer.playersleep -= 10
     $Trainer.playersaturation -= 10 #take from saturation
   else
     $Trainer.playerstamina -= 1
 case $Trainer.playersaturation
  when 0
    $Trainer.playerfood -= 7#take from hunger
    $Trainer.playerwater -= 7#take from drinking
 end
 end
 end
  pbFadeOutIn {
    scene = MiningGameScene3.new
    screen = MiningGame3.new(scene)
