...
module FileSave
	...
	# Version 19
	def self.dir_new(dir = DIR_SAVE_GAME, file = FILENAME_SAVE_GAME, type = "rxdata")
	...
	# Version 18
	def self.dir_old(dir = DIR_SAVE_GAME, file = FILENAME_SAVE_GAME, type = "rxdata")
		game_title = self.title
...
#----------------------------------#
# Set Deletion for module SaveData #
#----------------------------------#
module SaveData
      ...
      if !loadmenu
        ...
        if checkInput(Input::USE)
          ...
          # Mystery Gift
						...
						pbFadeOutIn { pbDownloadMysteryGift(@file_load[:player]) }
						@posinfor = 0
						choosePanelInfor
          # Language
						...
						$PokemonSystem.language = pbChooseLanguage
						pbLoadMessages('Data/' + Settings::LANGUAGES[$PokemonSystem.language][1])
						@file_load[:pokemon_system] = $PokemonSystem
						File.open(FileSave.name(@position+1), 'wb') { |file| Marshal.dump(@file_load, file) }
						@posinfor = 0
						choosePanelInfor
          end
        end
...
#-------------------------------------------------------------------------------
  ...
  def drawInfor(type,font=nil)
    ...
    textpos<<[_INTL("#{title}"), 32 + x, 16 + y, 0, TEXTCOLOR, TEXTSHADOWCOLOR]
    textpos<<[_INTL("Badges:"), 32 + x, 118 + y, 0, TEXTCOLOR, TEXTSHADOWCOLOR]
    textpos<<[trainer.badge_count.to_s, 206 + x, 118 + y, 1, TEXTCOLOR, TEXTSHADOWCOLOR]
    textpos<<[_INTL("Pokédex:"), 32 + x, 150 + y, 0, TEXTCOLOR, TEXTSHADOWCOLOR]
    textpos<<[trainer.pokedex.seen_count.to_s, 206 + x, 150 + y, 1, TEXTCOLOR, TEXTSHADOWCOLOR]
    textpos<<[_INTL("Time:"), 32 + x, 182 + y, 0, TEXTCOLOR, TEXTSHADOWCOLOR]
    hour = totalsec / 60 / 60
    min  = totalsec / 60 % 60
    if hour>0
      textpos<<[_INTL("{1}h {2}m",hour,min), 206 + x, 182 + y, 1, TEXTCOLOR, TEXTSHADOWCOLOR]
    else
      textpos<<[_INTL("{1}m",min), 206 + x, 182 + y, 1, TEXTCOLOR, TEXTSHADOWCOLOR]
    end
    if trainer.male?
      textpos<<[trainer.name, 112 + x, 70 + y , 0, MALETEXTCOLOR, MALETEXTSHADOWCOLOR]
    elsif
      textpos<<[trainer.name, 112 + x, 70 + y, 0, FEMALETEXTCOLOR, FEMALETEXTSHADOWCOLOR]
    else
      textpos<<[trainer.name, 112 + x, 70 + y, 0, TEXTCOLOR, TEXTSHADOWCOLOR]
    end
		mapid = self.fileLoad[:map_factory].map.map_id
    mapname = pbGetMapNameFromId(mapid)
    mapname.gsub!(/\\PN/,trainer.name)
    textpos<<[mapname, 386 + x, 16 + y, 1, TEXTCOLOR, TEXTSHADOWCOLOR]
    # Load menu
    if type==1
