class Player < Trainer
  ...
  class Pokedex
    ...
    def set_owned(species, should_refresh_dexes = true)
      ...
      @number_caught[species_id] = 0 if !@number_caught[species_id]
      @number_caught[species_id] += 1
      self.refresh_accessible_dexes if should_refresh_dexes
    end
    ...
    def number_caught(species)
      species_id = GameData::Species.try_get(species)&.species
      return false if species_id.nil?
      @number_caught[species_id] = 0 if !@number_caught[species_id]
      return @number_caught[species_id]
    end
    # Increase the Number of Pokemon of this species that the player has
    # caught or defeated
    # @param species [Symbol, GameData::Species] species to increase
    ...
    def number_weakness(species)
      species_id = GameData::Species.try_get(species)&.species
      return false if species_id.nil?
      @weakness_seen[species_id] = 0 if !@weakness_seen[species_id]
      return @weakness_seen[species_id]
    end
    ...
    # caught or defeated
    # @param species [Symbol, GameData::Species] species to increase
    def register_weakness(species)
      species_id = GameData::Species.try_get(species)&.species
      return if species_id.nil?
      @weakness_seen[species_id] = 0 if !@weakness_seen[species_id]
      @weakness_seen[species_id] += 1
    end
    
    
    def number_evolved(species)
      species_id = GameData::Species.try_get(species)&.species
      return false if species_id.nil?
      @evolved_seen[species_id] = 0 if !@evolved_seen[species_id]
      return @evolved_seen[species_id]
    end
    ...
    # caught or defeated
    # @param species [Symbol, GameData::Species] species to increase
    def register_evolved(species)
      species_id = GameData::Species.try_get(species)&.species
      return if species_id.nil?
      @evolved_seen[species_id] = 0 if !@evolved_seen[species_id]
      @evolved_seen[species_id] += 1
    end
    
    def number_moves(species,move)
      species_id = GameData::Species.try_get(species)&.species
      move_data = GameData::Move.try_get(move)
      return false if species_id.nil?
      return false if move_data.nil?
      @moves_seen[species_id] = 0 if !@moves_seen[species_id]
      return @moves_seen[species_id]
    end
    ...
    # caught or defeated
    # @param species [Symbol, GameData::Species] species to increase
    def register_move(species,move)
      species_id = GameData::Species.try_get(species)&.species
      move_data = GameData::Move.try_get(move_id)&.move
      return if species_id.nil?
      return false if move_data.nil?
      @moves_seen[species_id] = 0 if !@moves_seen[species_id]
      @moves_seen[species_id] += 1
    end
