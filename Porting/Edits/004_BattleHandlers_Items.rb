...
BattleHandlers::HPHealItem.add(:ORANMASH,
  proc { |item,battler,battle,forced|
...
BattleHandlers::HPHealItem.add(:APPLE,
  proc { |item,battler,battle,forced|
    next false if !battler.canHeal?
    next false if !forced && battler.hp>battler.totalhp/2
    itemName = GameData::Item.get(item).name
...
BattleHandlers::HPHealItem.add(:ARGOSTBERRY,
   proc { |item,battler,battle,forced|
    next false if !battler.canHeal?
    next false if !forced && !battler.canConsumeArgBerry?(false)
    battle.pbCommonAnimation("EatBerry",battler) if !forced
    battler.hp = 1
    itemName = GameData::Item.get(item).name
    if forced
      PBDebug.log("[Item triggered] Forced consuming of #{itemName}")
      battle.pbDisplay(_INTL("{1}'s recovered from fainting.",battler.pbThis))
    else
      battle.pbDisplay(_INTL("{1} recovered from fainting!",pokemon.name))
    end
    next true
  }
)

BattleHandlers::HPHealItem.add(:ENERGYPOWDER,
   proc { |item,battler,battle,forced|
    next false if !battler.canHeal?
    next false if !forced && !battler.canConsumeArgBerry?(false)
    battle.pbCommonAnimation("EatBerry",battler) if !forced
...
BattleHandlers::DamageCalcUserItem.copy(:CHARCOAL,:COAL,:FLAMEPLATE)
...
BattleHandlers::AccuracyCalcTargetItem.add(:EVIOLITE,
  proc { |item,mods,user,target,move,type|
  if target.pokemon.species_data.get_evolutions(true).length > 0
    mods[:accuracy_multiplier] *= 1.025
  end
  }
)

BattleHandlers::PriorityBracketUseItem.add(:EVIOLITE,
  proc { |item,battler,battle|
    battle.pbCommonAnimation("UseItem",battler)
    battle.pbDisplay(_INTL("{1}'s {2} let it move first!",battler.pbThis,battler.itemName))
  }
