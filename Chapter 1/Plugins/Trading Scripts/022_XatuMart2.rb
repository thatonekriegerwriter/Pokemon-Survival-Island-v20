...
#
# *to add an item of your own just add it to the RECIPEMART2 array.
...
# This goes through the RECIPEMART2 array and adds the true or false value from it
# to the $game_variables[CRAFTVAR] array
    for i in 0...RECIPEMART2.length
     $isUnlocked[i] = RECIPEMART2[i][4]
    end
...
class TradingMart2Scene

  def initialize
    @close = $exit
    @select=3
    @item=0
    @mat1=RECIPEMART2[@item][2]? RECIPEMART2[@item][2][0] : -1 # the amount for first item
    @mat2=RECIPEMART2[@item][3]? RECIPEMART2[@item][3][0] : -1 # the amount for first item
    @cost1=RECIPEMART2[@item][2]? RECIPEMART2[@item][2][1] : 0 # the amount for first item
    @cost2=RECIPEMART2[@item][3]? RECIPEMART2[@item][3][1] : 0 # the amount for first item
    @amount=RECIPEMART2[@item][1] # the amount for the first item made
    ...
    def input
      ...
      when 3
        ...
        if $game_variables[CRAFTVAR][@item]
            ...
            @mat2=RECIPEMART2[@item][3]? RECIPEMART2[@item][3][0] : -1
            @cost1=RECIPEMART2[@item][2]? RECIPEMART2[@item][2][1] : 0
            @cost2=RECIPEMART2[@item][3]? RECIPEMART2[@item][3][1] : 0
            @amount=RECIPEMART2[@item][1]
          else
        ...
        # When pressing Right
        if Input.trigger?(Input::RIGHT)  && @item < RECIPEMART2.length-1
          @item+=1
        elsif Input.trigger?(Input::RIGHT)  && @item ==RECIPEMART2.length-1 # Make it run though the selection after last item.
          @item = 0
        end
        ...
        when 2 
          if $game_variables[CRAFTVAR][@item]
            if $PokemonBag.pbQuantity(@mat1)<@cost1 || (@mat2!=-1 && $PokemonBag.pbQuantity(@mat2) <@cost2) #Seth Edited 
              ...
              $PokemonBag.pbStoreItem(RECIPEMART2[@item][0],@amount)
  ...
  def text
    ...
    @text3=_INTL("{1} / {2}", @item + 1, RECIPEMART2.length)
