...
def pbDisplayNameWindow(params)
  ...
  colorBase    = colorToRgb32(MessageConfig::LIGHT_TEXT_MAIN_COLOR)
  colorBase    = colorToRgb32(MessageConfig::DARK_TEXT_MAIN_COLOR) if isDark==true
  colorBase    = params[3]  if !params[3].nil?
  colowShadow  = colorToRgb32(MessageConfig::LIGHT_TEXT_SHADOW_COLOR)
  colorShadow  = colorToRgb32(MessageConfig::DARK_TEXT_SHADOW_COLOR) if isDark==true
...
def pbMessageDisplay(msgwindow,message,letterbyletter=true,commandProc=nil)
  ...
  text.gsub!(/\\[Pp][Nn]/,$player.name) if $player
  text.gsub!(/\\[Pp][Mm]/,_INTL("${1}",$player.money.to_s_formatted)) if $player
  text.gsub!(/\\[Nn]/,"\n")
  text.gsub!(/\\\[([0-9A-Fa-f]{8,8})\]/){ "<c2="+$1+">" }
  text.gsub!(/\\[Pp][Gg]/,"\\b") if $player && $player.male?
  text.gsub!(/\\[Pp][Gg]/,"\\r") if $player && $player.female?
  text.gsub!(/\\[Pp][Oo][Gg]/,"\\r") if $player && $player.male?
  text.gsub!(/\\[Pp][Oo][Gg]/,"\\b") if $player && $player.female?
  text.gsub!(/\\[Pp][Gg]/,"")
  text.gsub!(/\\[Pp][Oo][Gg]/,"")
