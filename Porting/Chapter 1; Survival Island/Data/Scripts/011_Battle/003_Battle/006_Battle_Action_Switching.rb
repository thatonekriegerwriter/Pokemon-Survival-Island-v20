class PokeBattle_Battle
  ...
  def pbCanSwitchLax?(idxBattler,idxParty,partyScene=nil)
    ...
    if party[idxParty].fainted?
      partyScene.pbDisplay(_INTL("{1} is out cold!",
         party[idxParty].name)) if partyScene
      return false
    end
    if pbFindBattler(idxParty,idxBattler)
      partyScene.pbDisplay(_INTL("{1} is already fighting!",
         party[idxParty].name)) if partyScene
      return false
    end
    return true
  ...
  def pbMessageOnRecall(battler)
    if battler.pbOwnedByPlayer?
      if battler.hp<=battler.totalhp/4
        pbDisplayBrief(_INTL("That's enough, {1}! Return!",battler.name))
      elsif battler.hp<=battler.totalhp/2
        pbDisplayBrief(_INTL("OK, {1}! Return!",battler.name))
      elsif battler.turnCount>=5
        pbDisplayBrief(_INTL("{1}, Return!",battler.name))
      elsif battler.turnCount>=2
        pbDisplayBrief(_INTL("{1}, Quick, return!",battler.name))
      else
        pbDisplayBrief(_INTL("{1}, switch out! Come back!",battler.name))
        pbDisplayBrief(_INTL("That's enough, {1}! Return!",battler.name)) if battler.happiness > 200
      end
    else
  ...
  def pbOnActiveOne(battler)
    ...
    if battler.opposes? && battler.shadowPokemon?
      ...
      pbDisplay(_INTL("There's something off about that POKeMON!"))
    end
