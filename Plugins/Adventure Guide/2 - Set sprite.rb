module AdventureGuide
	class Show
		...
		#------------#
		# Image
		def create_sprite(spritename, filename, vp, dir = "Adventure Guide")
		...
		def set_sprite(spritename, filename, dir = "Adventure Guide")
			file = dir ? "Graphics/Pictures/#{dir}/#{filename}" : "Graphics/Pictures/#{filename}"
			@sprites["#{spritename}"].bitmap = Bitmap.new(file)
		end
		# Set ox, oy
		...
		# Clear
		def clearTxt(bitmap) = @sprites["#{bitmap}"].bitmap.clear
		#------------------------------------------------------------------------------#
		# Set SE for input
		#------------------------------------------------------------------------------#
