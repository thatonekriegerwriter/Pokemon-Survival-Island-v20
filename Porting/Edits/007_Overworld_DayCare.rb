...
def pbDayCareGetCompat
  ...
  # cannot breed
  return 0 if (egg_groups1 & egg_groups2).length == 0
  ...
  ret += rand(4) if $game_variables[50]==10
  return ret
end
...
def pbDayCareGenerateEgg
  ...
  if pkmn0.female? || ditto0
    mother = pkmn0
    father = pkmn1
    fgrandfather = father.family.father if father.family && father.family.father
    fgrandmother = father.family.mother if father.family && fatherfamily.father
    mgrandfather = mother.family.mother if mother.family && mother.family.father
    mgrandmother = mother.family.father if mother.family && mother.family.father
    babyspecies = (ditto0) ? father.species : mother.species
    babyspecies = [mother.species, father.species,fgrandfather.species,fgrandmother.species,mgrandfather.species,mgrandmother.species][rand(6)] if father.family && father.family.father && mother.family && mother.family.mother && mother.family.father  && father.family.mother
    babyspecies = [mother.species, father.species][rand(2)] if mother.hasItem?(:GOLDLEAF) || father.hasItem?(:GOLDLEAF)
    babyspecies = [mother.species, mother.species, mother.species, father.species][rand(4)] if mother.hasItem?(:GOLDLEAF) || !father.hasItem?(:GOLDLEAF)
    babyspecies = [mother.species, father.species, father.species, father.species][rand(4)] if !mother.hasItem?(:GOLDLEAF) || father.hasItem?(:GOLDLEAF)
  else
    mother = pkmn0
    father = pkmn1
    fgrandfather = father.family.father if father.family && father.family.father
  ...
  # Add egg to party
  if $game_switches[298]==true
   if $game_switches[288]==false
    $game_variables[4994]=egg
      $game_switches[288]=true
   elsif $game_switches[289]==false
    $game_variables[4995]=egg
      $game_switches[289]=true
   elsif $game_switches[290]==false
    $game_variables[4996]=egg
      $game_switches[290]=true
   elsif $game_switches[291]==false
    $game_variables[4997]=egg
      $game_switches[291]=true
   elsif $game_switches[292]==false
    $game_variables[4998]=egg
      $game_switches[292]=true
   else $game_switches[293]==false
    $game_variables[4999]=egg
      $game_switches[293]=true
   end
  else
...
Events.onStepTaken += proc { |_sender,_e|
  ...
  if deposited==2 && $PokemonGlobal.daycareEgg==0
    ...
    if $PokemonGlobal.daycareEggSteps==256
      ...
      if $game_switches[296]==true  && GameData::Item.exists?(:OVALCHARM) && $PokemonBag.pbHasItem?(:OVALCHARM)
        compatval = [20,60,80,88][pbDayCareGetCompat]
      elsif $game_switches[296]==true
