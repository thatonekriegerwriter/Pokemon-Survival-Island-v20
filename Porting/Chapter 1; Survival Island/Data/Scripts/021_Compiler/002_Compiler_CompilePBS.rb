    ...
    pbCompilerEachCommentedLine(path) { |line, line_no|
      if line[/^\s*(\w+)\s*=\s*(.*)$/]   # Of the format XXX = YYY
        ...
        berry_plant_hash = {
          ...
          :weedchance      => line[10],
          :weeded          => line[11],
          :speciesallele   => line[12],
          :growthallele    => line[13],
          :dryallele       => line[14],
          :minyieldallele  => line[15],
          :maxyieldallele  => line[16]
        }
    ...
    File.open(path, "rb") { |f|
      ...
      pbEachFileSection(f) { |contents, species_number|
        ...
        species_hash = {
          ...
          :food                  => contents["Food"],
          :water                 => contents["Water"],
          :sleep                 => contents["Sleep"],
    ...
    File.open(path, "rb") { |f|
      ...
      pbEachFileSection2(f) { |contents, section_name|
        ...
        species_hash = {
          ...
          :loyalty               => contents["Loyalty"] || base_data.loyalty,
          :food                  => contents["Food"] || base_data.food,
          :water                 => contents["Water"] || base_data.water,
          :sleep                 => contents["Sleep"] || base_data.sleep,
          :moves                 => moves,
      ...
      if line[/^\s*\[\s*(.+)\s*\]\s*$/]
        ...
        when "Food"
          if property_value > 100
            raise _INTL("Bad happiness: {1} (must be 0-100).\r\n{2}", property_value, FileLineData.linereport)
          end
        when "Water"
          if property_value > 100
            raise _INTL("Bad happiness: {1} (must be 0-100).\r\n{2}", property_value, FileLineData.linereport)
