...
#
# *to add an item of your own just add it to the RECIPEMART3 array.
...
# This goes through the RECIPEMART3 array and adds the true or false value from it
# to the $game_variables[CRAFTVAR] array
    for i in 0...RECIPEMART3.length
     $isUnlocked[i] = RECIPEMART3[i][4]
    end
...
class TradingMart3Scene

  def initialize
    @close = $exit
    @select=3
    @item=0
    @mat1=RECIPEMART3[@item][2]? RECIPEMART3[@item][2][0] : -1 # the amount for first item
    @mat2=RECIPEMART3[@item][3]? RECIPEMART3[@item][3][0] : -1 # the amount for first item
    @cost1=RECIPEMART3[@item][2]? RECIPEMART3[@item][2][1] : 0 # the amount for first item
    @cost2=RECIPEMART3[@item][3]? RECIPEMART3[@item][3][1] : 0 # the amount for first item
    @amount=RECIPEMART3[@item][1] # the amount for the first item made
    ...
    @sprites["Item_icon"].setBitmap(GameData::Item.icon_filename(GameData::Item.try_get(RECIPEMART3[@item][0])))
    ...
    self.openTradingMart3Scene
  end
  
  def openTradingMart3Scene
    ...
    def input
      ...
      when 3
        ...
        if $game_variables[CRAFTVAR][@item]
            ...
            @sprites["Item_1_icon"].setBitmap(RECIPEMART3[@item][2]? GameData::Item.icon_filename(GameData::Item.try_get(RECIPEMART3[@item][2][0])) : "") # Vendily
            @sprites["Item_2_icon"].setBitmap(RECIPEMART3[@item][3]? GameData::Item.icon_filename(GameData::Item.try_get(RECIPEMART3[@item][3][0])) : "") # Vendily
            @sprites["Item_1_icon"].opacity= RECIPEMART3[@item][2] ? 255 : 0
            @sprites["Item_2_icon"].opacity=RECIPEMART3[@item][3] ? 255 : 0
            @mat1=RECIPEMART3[@item][2]? RECIPEMART3[@item][2][0] : -1
            @mat2=RECIPEMART3[@item][3]? RECIPEMART3[@item][3][0] : -1
            @cost1=RECIPEMART3[@item][2]? RECIPEMART3[@item][2][1] : 0
            @cost2=RECIPEMART3[@item][3]? RECIPEMART3[@item][3][1] : 0
            @amount=RECIPEMART3[@item][1]
          else
        ...
        # When pressing Right
        if Input.trigger?(Input::RIGHT)  && @item < RECIPEMART3.length-1
          @item+=1
        elsif Input.trigger?(Input::RIGHT)  && @item ==RECIPEMART3.length-1 # Make it run though the selection after last item.
          @item = 0
        end
        ...
        when 2 
          if $game_variables[CRAFTVAR][@item]
            if $PokemonBag.pbQuantity(@mat1)<@cost1 || (@mat2!=-1 && $PokemonBag.pbQuantity(@mat2) <@cost2) #Seth Edited 
              ...
              $PokemonBag.pbStoreItem(RECIPEMART3[@item][0],@amount)
              $PokemonBag.pbDeleteItem(@mat1,@cost1)
              if @mat2!=-1
                $PokemonBag.pbDeleteItem(@mat2,@cost2)
              end
              self.text
              Kernel.pbMessage(_INTL("{1} and {2} were traded for each other.", GameData::Item.get(GameData::Item.try_get(@mat1)).name, GameData::Item.get(GameData::Item.try_get(RECIPEMART3[@item][0])).name))
            end
          else
  ...
  def text
    ...
    @text3=_INTL("{1} / {2}", @item + 1, RECIPEMART3.length)
