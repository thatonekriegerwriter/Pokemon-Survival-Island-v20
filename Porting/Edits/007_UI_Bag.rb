...
class PokemonBagScreen
  ...
  def pbStartScreen
    ...
    loop do
      ...
      commands[cmdEat = commands.length]       = _INTL("Use (Self)") if itm.is_foodwater? || itm.is_berry?
      ...
      if ItemHandlers.hasOutHandler(item) || (itm.is_machine? && $Trainer.party.length>0)
        if ItemHandlers.hasUseText(item)
          ...
          commands[cmdUse = commands.length]    = _INTL("Use (Other)")
        end
      end
  ...
  itm = GameData::Item.get(item)
        pbMessage(_INTL("{1}",itm.field_use))
