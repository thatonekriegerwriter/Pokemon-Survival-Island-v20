...
def pbLifeDewPartySkill(pkmn, movename, screen, party, party_idx)
  ...
  if pkmn.hp == 0
    ...
    party.each do |p|
	 ...
	 if $player
      amt = [($player.playermaxhealth / 4).floor, 1].max
	 increaseHealth(amt)
      ret += 1
	 end
    case ret
    when 0 then screen.scene.pbDisplay(_INTL("It won't have any effect."))
    else        screen.scene.pbDisplay(_INTL("{1} restored the everyone's HP!", pkmn.name))
    end
  end
  screen.scene.pbSelect(party_idx)
  screen.scene.pbRefresh
...
def pbStatusPartySkill(pkmn, movename, screen, party, party_idx)
  ...
  if pkmn.hp == 0
	...
	if $player && $player.playerstateffect!=:NONE
	$player.playerstateffect=:NONE
      ret += 1
	end
    case ret
    when 0 then screen.scene.pbDisplay(_INTL("It won't have any effect."))
