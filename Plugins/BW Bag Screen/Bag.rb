...
class PokemonBag_Scene
  ...
  def showPocketAnimation
    if $player.female?
      @sprites["bagsprite"].setBitmap("Graphics/Pictures/Bag/bag_#{@bag.last_viewed_pocket}_fm") 
      pbWait(6) 
      @sprites["bagsprite"].setBitmap("Graphics/Pictures/Bag/bag_#{@bag.last_viewed_pocket}_f")
    else
      @sprites["bagsprite"].setBitmap("Graphics/Pictures/Bag/bag_#{@bag.last_viewed_pocket}m")
      pbWait(6)
...
#            end
          ...
          elsif Input.triggerex?(0x31) #"Items"
            newpocket = itemwindow.pocket
            loop do
              newpocket = 1
              break if !@choosing || newpocket == itemwindow.pocket
...
class PokemonBagScreen
  ...
  def pbWithdrawItemScreen(storage)
    ...
    loop do
      ...
      if @bag.can_add?(item, qty)
        ...
        itemname = (dispqty.to_i > 1) ? itm.name_plural : itm.name
        pbDisplay(_INTL("Withdrew {1} {2}.", dispqty.to_i, itemname))
      else
  ...
  def pbDepositItemScreen(storage)
    @scene.pbStartScene(@bag)
    loop do
      item = @scene.pbChooseItem
      break if !item
