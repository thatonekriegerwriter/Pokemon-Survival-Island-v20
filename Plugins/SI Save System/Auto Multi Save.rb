...
module SaveData
  ...
  def self.getSlots
    return TESTING_SLOTS if TESTING_MODE == true && $DEBUG
    if File.file?(self.get_full_path(HARDCORE_SLOTS))
      summary = self.read_summary(HARDCORE_SLOTS)
      return [HARDCORE_SLOTS] if summary && summary[:hardcore] == true
    end
  ...
  def self.get_newest_save_slot
    ...
    self.each_slot do |file_slot|
      ...
      summary = self.read_summary(file_slot)
      save_time = summary[:last_time_saved] || Time.at(1)
  ...
  #-----------------------------------------------------------------------------
  # Save summaries — a small sidecar file with just enough data to build the
  # save-select previews (last saved time, map, playtime, and basic player
  # stats), so menus that only need a preview don't have to Marshal.load the
  # entire save (full Pokédex, party movesets/IVs/EVs, bag, map flags, etc.)
  # just to show a few lines of text. Only the actual "load this save" flow
  # should ever need the real, full save data.
  #-----------------------------------------------------------------------------

  def self.summary_path(slot)
    "#{self.get_full_path(slot)}.summary"
  end

  # Builds a summary hash from a live or loaded Player object. Used both when
  # writing a fresh summary at save time (from the live $player) and when
  # rebuilding a missing summary from a fully-loaded old save (see
  # read_summary below) — both cases just need "some Player object".
  #
  # party is stored as the real Pokemon objects (not a stripped-down stand-in)
  # so the preview's party icons keep working with zero changes to
  # pbSetParty/PokemonIconSprite — a party of up to 6 Pokemon is negligible
  # next to what actually makes a save file big (Pokédex, bag, per-map flags).
  def self.build_summary(player, map_id, frame_count, hardcore)
    classy = player.playerclass.respond_to?(:name) ? player.playerclass.name : player.playerclass.to_s
    gender = player.male? ? :male : (player.female? ? :female : :other)
    {
      last_time_saved:  player.last_time_saved,
      map_id:            map_id,
      frame_count:       frame_count,
      name:              player.name,
      gender:            gender,
      playerclass_name: classy,
  ...
  def self.write_summary(slot, summary)
    File.open(self.summary_path(slot), "wb") { |f| Marshal.dump(summary, f) }
  end

  # Reads a slot's summary. If one doesn't exist yet (a save made before this
  # feature existed, or a summary write that failed), falls back to a full
  # load exactly once and writes the summary immediately afterward, so every
  # later read of this slot is fast.
  def self.read_summary(slot)
    full_path = self.get_full_path(slot)
    return nil unless File.file?(full_path)
    path = self.summary_path(slot)
    if File.file?(path)
      begin
        return File.open(path, "rb") { |f| Marshal.load(f) }
      rescue StandardError
        # fall through and rebuild it below
      end
    end
    save_data = self.read_from_file(full_path)
    summary = self.build_summary(save_data[:player], save_data[:map_factory].map.map_id,
                                  save_data[:frame_count] || 0, save_data[:global_metadata].hardcore)
    self.write_summary(slot, summary)
    summary
  end
  ...
  def self.delete_this_save(file_path=nil)
    if file_path
      ...
      File.delete(summary_path) if File.file?(summary_path)
    else
      self.each_slot do |slot|
        full_path = self.get_full_path(slot)
        File.delete(full_path) if File.file?(full_path)
        summary_path = self.summary_path(slot)
        File.delete(summary_path) if File.file?(summary_path)
      end
    end
  end
  ...
  def refresh
    ...
    if @refreshBitmap
      ...
      if @isContinue
        ...
        textpos.push([@trainer.playerclass.name, 206, 118, 1, TEXTCOLOR, TEXTSHADOWCOLOR])
        ...
        # NOTE: this is a full SaveData.read_from_file, run on every refresh — i.e. every
        # time the selected panel changes, not just once. Same class of issue as
        # everywhere else in this file, but fixing it here needs a new :playermode
        # field on the summary plus rewiring how PokemonLoad_Scene builds these
        # panels — bigger, separate work, so left alone for now.
        save_data = SaveData.read_from_file((SaveData.get_full_path(@savefile)))
        hardcore = save_data[:global_metadata].hardcore == true
        demo = save_data[:pokemon_system].playermode == 0
        suffix = nil
        suffix = " (DH)" if hardcore && demo
        suffix = " (D)" if demo && !hardcore
        suffix = " (H)" if hardcore && !demo
        if suffix
          # suffixed text sits at a different x than the plain filename so the
          # "File X" part stays visually aligned once the suffix is appended
          textpos.push([_INTL("{1}{2}", @savefile.to_s, suffix), 229, 16, 1, TEXTCOLOR, TEXTSHADOWCOLOR])
        else
          textpos.push([@savefile, 216, 16, 1, TEXTCOLOR, TEXTSHADOWCOLOR])
        end
        mapname = pbGetMapNameFromId(@mapid)
        mapname.gsub!(/\\PN/, @trainer.name)
        mapname = "Dreamyard" if mapname.include?("(Folder)")
        text = hardcore ? mapname : "#{mapname} ->"
...
class PokemonLoad_Scene
  def pbStartScene(commands, show_continue, savefile, trainer, frame_count, stats, map_id)
    ...
    @stars = (1..5).map do |i|
      star = IconSprite.new(5, 23 + (i * 12), @viewport)
      star.visible = false
      star.setBitmap("Graphics/Pictures/stars1.png")
      star
    end
  ...
  def pbChoose(commands, continue_idx)
    ...
    loop do
      ...
      if Input.trigger?(Input::USE) || Input.trigger?(Input::MOUSELEFT)
        @stars.each { |s| s.visible = false }
        return @sprites["cmdwindow"].index
      elsif @sprites["cmdwindow"].index == continue_idx
        # a missing hardcore save is treated as "not hardcore-restricted" rather
        # than left to crash on a full load of a file that may not exist
        hardcore_summary = SaveData.read_summary(SaveData::HARDCORE_SLOTS)
        not_hardcore_only = hardcore_summary.nil? || hardcore_summary[:hardcore] == false
        if Input.trigger?(Input::LEFT) && not_hardcore_only
          return -3
        elsif Input.trigger?(Input::RIGHT) && not_hardcore_only
          return -2
...
class PokemonSaveScreen
  ...
  def getSaveInfoBoxContents(slot)
    ...
    summary = SaveData.read_summary(slot)

    # Last save time
    time = summary[:last_time_saved]
    if time
    ...
    # Map name
    map_str = pbGetMapNameFromId(summary[:map_id])

    # Elapsed time
    totalsec = (summary[:frame_count] || 0) / Graphics.frame_rate
    hour = totalsec / 60 / 60
    min = totalsec / 60 % 60
    if hour > 0
...
module Game
  ...
  def self.save(slot=nil, auto=false, safe: false)
    ...
    begin
      summary = SaveData.build_summary($player, $game_map.map_id, Graphics.frame_count, $PokemonGlobal.hardcore)
      SaveData.write_summary(slot, summary)
    rescue StandardError
      # the summary is only a cache for menu previews — losing it just means the
      # next read of this slot rebuilds it from a full load, so this must never
      # turn an otherwise-successful save into a reported failure
    end
    return true
  end
  ...
  def self.auto_save
    ...
    if $PokemonGlobal.hardcore == true
      slot = SaveData::HARDCORE_SLOTS
      summary = SaveData.read_summary(slot)
      save_time = summary ? (summary[:last_time_saved] || Time.at(1)) : Time.at(1)
