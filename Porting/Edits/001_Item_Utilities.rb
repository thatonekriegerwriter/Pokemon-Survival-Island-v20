...
def pbRaiseHappinessAndLowerEV(pkmn,scene,stat,messages)
  ...
  if h
    pkmn.changeHappiness("evberry",pkmn)
    pkmn.changeLoyalty("evberry",pkmn)
  end
  if e
    pkmn.ev[stat] -= 10
...
def pbBikeCheck
  if $PokemonGlobal.surfing || $PokemonGlobal.diving ||
    ...
    pbMessage(_INTL("There's a time and a place for everything. Now is not that time."))
    return false
  end
...
def pbLearnMove(pkmn,move,ignoreifknown=false,bymachine=false,&block)
  ...
  loop do
    ...
    if forgetmove>=0
      ...
      pbMessage(_INTL("\\se[]{1} learned {2}!\\se[Pkmn move learnt]",pkmnname,movename),&block)
      pkmn.changeHappiness("machine",pkmn) if bymachine
      pkmn.changeLoyalty("machine",pkmn) if bymachine
      return true
