...
SaveData.register(:graphics_manager) do
  load_before_compile #load_before_compile?
  ensure_class :GraphicsManager
  save_value { $graphics_manager }
  load_value { |value| 
    if value.nil?
    puts "Making a new graphics manager"
   $graphics_manager = GraphicsManager.new 
	else
   $graphics_manager = value 
    end 
   }
  new_game_value { GraphicsManager.new }
end
...
module Graphics
  ...
  def self.width
	...
	width += $graphics_manager.width if $graphics_manager 
	return width
  end
  def self.height
    #height = 480
  ...
  def self.fps
    self.average_frame_rate.round(2)
  end
...
class Viewport
  ...
  define_method(:initialize) do |x, y, width, height|
    ...
    def ox
	  ox = __orig_ox
	  ox += $graphics_manager.offset_x if $graphics_manager
      return ox
    end

    def oy
	  oy = __orig_oy
	  oy += $graphics_manager.offset_y if $graphics_manager
      return oy
    end
...
def pbSetResizeFactor(factor)
  if !$ResizeInitialized
    Graphics.resize_screen(Settings::SCREEN_WIDTH + $graphics_manager.offset_x, Settings::SCREEN_HEIGHT + $graphics_manager.offset_y)
...
def pbConnectionsEditor
  pbCriticalCode {
    Graphics.resize_screen(Settings::SCREEN_WIDTH + 288 + $graphics_manager.offset_x, Settings::SCREEN_HEIGHT + 288 + $graphics_manager.offset_y)
    pbSetResizeFactor(1)
