...
when EnumOption
  if @options[index].values.length > 1
    ...
    spacing = (
      rect.width - rect.x - optionwidth - totalwidth
  ...
  elsif @options[index].values.length == 1
    value = @options[index].values[0]
    ...
    pbDrawShadowText(
      ...
      rect.height,
      value,
      SEL_VALUE_BASE_COLOR,
      SEL_VALUE_SHADOW_COLOR
    )
  end
...
when NumberOption
  value = _INTL(
    "Type {1}/{2}",
    @options[index].lowest_value + self[index],
...
when SliderOption
  ...
  sliderlength =
    rect.width - rect.x - optionwidth - self.contents.text_size(value).width
...
when SelectOption
  # SelectOptions are submenu entries and therefore have no value.

else
  if @options[index].values && !@options[index].values.empty?
...
if self.active && self.index < @options.length
  if Input.repeat?(Input::LEFT)
    ...
    # One-value options are actions, not settings.
    unless option.is_a?(EnumOption) && option.values.length <= 1
      self[self.index] = option.prev(self[self.index])
      dorefresh = true
      @value_changed = true
    end
    ...
    # One-value options are actions, not settings.
    unless option.is_a?(EnumOption) && option.values.length <= 1
      self[self.index] = option.next(self[self.index])
      dorefresh = true
      @value_changed = true
    end
  end
...
def get_unique_parameters(hash, option_id)
return hash["parameters"] if $PokemonGlobal.nil?


case option_id
when :hardcore
  return [_INTL("Enabled")] if $PokemonGlobal.hardcore == true

when :permadeath
  return [_INTL("Enabled")] if nuzlocke_has?(:PERMADEATH)

when :one_route
  return [_INTL("Enabled")] if nuzlocke_has?(:ONEROUTE)

when :nicknameforcing
  return [_INTL("Enabled")] if nuzlocke_has?(:NICKNAMES)

when :sleepforcing
  return [_INTL("Enabled")] if nuzlocke_has?(:AFULLEIGHTHOURS)

when :nocatchforcing
  return [_INTL("Enabled")] if nuzlocke_has?(:NOOVCATCHING)

when :nostatueforcing
  return [_INTL("Enabled")] if nuzlocke_has?(:NOSTATUES)

when :pkmnfoodandwater
  return [_INTL("Enabled")] if nuzlocke_has?(:SURVIVALMODE)

when :dups_clause
  return [_INTL("Disabled")] if !nuzlocke_has?(:DUPSCLAUSE)

when :static_clause
  return [_INTL("Disabled")] if !nuzlocke_has?(:STATIC)

when :shiny_clause
  return [_INTL("Disabled")] if !nuzlocke_has?(:SHINY)
end
...
def has_submenu?(option_id)
return false if option_id.nil?


MenuHandlers.each_available(:options_menu) do |id, hash, _name|
  hash["parent"] = :main if hash["parent"].nil?
  return true if hash["parent"] == option_id
end
...
return nil if menu == :main


parent = :main

MenuHandlers.each_available(:options_menu) do |option_id, hash, _name|
  hash["parent"] = :main if hash["parent"].nil?
...
def find_option_index(menu, option_id)
return 0 if option_id.nil?
...
def change_menu(menu, index = 0)
if @sprites["option"] && $PokemonSystem.currentList
  old_menu = $PokemonSystem.currentList
  @menu_positions[old_menu] = {
    :index   => @sprites["option"].index,
    :top_row => @sprites["option"].top_row
  }
end
$PokemonSystem.currentList = menu
@options, @hashes, @option_ids = build_menu(menu)

case menu
when :main
  title = _INTL("Options")
when :gameplay_menu
  title = _INTL("Gameplay Options")
when :gameplay_menu2
  title = _INTL("Gameplay Options")
when :ui_menu
  title = _INTL("UI Options")
when :music_menu
  title = _INTL("Music Options")
when :challenges_menu
  title = _INTL("Challenges...")
when :controls_menu
  title = _INTL("Controls Options")
else
  title = _INTL("Options")
end

@sprites["title"].text = title
@sprites["option"].set_options(@options)
@options.length.times do |i|
  value = @options[i].get
  value = 0 if value.nil?

  # Keep the value inside the option's valid range.
  if @options[i].values && !@options[i].values.empty?
    value = 0 if value < 0
    value = @options[i].values.length - 1 if value >= @options[i].values.length
  elsif @options[i].is_a?(NumberOption)
    value = 0 if value < @options[i].lowest_value
    value = @options[i].highest_value - @options[i].lowest_value if
      value > @options[i].highest_value
  elsif @options[i].is_a?(SliderOption)
    value = 0 if value < @options[i].lowest_value
    value = @options[i].highest_value - @options[i].lowest_value if
      value > @options[i].highest_value
  end

  @sprites["option"].setValueNoRefresh(i, value)
end

saved = @menu_positions[menu]

if saved
  index   = saved[:index]
  top_row = saved[:top_row]
else
  top_row = 0
end
...
index = 0 if index < 0
index = @options.length if index > @options.length

@sprites["option"].set_index(index)
@sprites["option"].top_row = top_row
...
#---------------------------------------------------------------------------
def pbSceneControls
change_menu($PokemonSystem.currentList, @sprites["option"].index)
end

#---------------------------------------------------------------------------

# Updates the description for the currently selected option.
...
#---------------------------------------------------------------------------
def go_back
current_menu = $PokemonSystem.currentList
parent = parent_menu(current_menu)
...
# Find the submenu entry in the parent so the cursor returns to it.
cursor = find_option_index(parent_id, current_menu)

change_menu(parent_id, cursor)
...
index = -1
  ...
  loop do
    ...
    #---------------------------------------------------------------------
    # Commit normal left/right changes.
    #---------------------------------------------------------------------
    ...
    if Input.trigger?(Input::BACK)
      ...
      # Restore memorized BGM if a music option was previewed.
      if $game_temp.memorized_bgm &&
         $game_system.is_a?(Game_System)

        $game_system.bgm_pause
        $game_system.bgm_position =
          $game_temp.memorized_bgm_position
        $game_system.bgm_resume(
          $game_temp.memorized_bgm
        )
      ...
      # Main menu exits.
      break if $PokemonSystem.currentList == :main

      # Otherwise return to the actual parent.
      go_back
      index = -1
      next
    ...
    #---------------------------------------------------------------------
      ...
      # Submenu.
      #---------------------------------------------------------------
      if has_submenu?(option_id)
        pbPlayDecisionSE
        change_menu(option_id, 0)
        index = -1
        next
      end

      #---------------------------------------------------------------
      # Controls submenu is an external UI.
      #---------------------------------------------------------------
      ...
      #---------------------------------------------------------------
      # One-value EnumOptions are actions.
      #---------------------------------------------------------------
      if option.is_a?(EnumOption) &&
        ...
        # Challenge options can change which options exist and/or
        # what their displayed value is.
        refresh_options(
          index
        )
...
# Rebuilds the current menu while preserving the cursor where possible.

#---------------------------------------------------------------------------
def refresh_options(previous_index = nil)
current_menu = $PokemonSystem.currentList
old_option_id = nil
...
if previous_index &&
   previous_index >= 0 &&
   previous_index < @option_ids.length
  old_option_id = @option_ids[previous_index]
end

@options, @hashes, @option_ids = build_menu(current_menu)
...
@options.length.times do |i|
  ...
  if @options[i].values && !@options[i].values.empty?
    value = 0 if value < 0
    value = @options[i].values.length - 1 if
      value >= @options[i].values.length
  end
...
if old_option_id
  found_index = @option_ids.index(old_option_id)
  new_index = found_index if found_index
elsif previous_index
  new_index = previous_index
  new_index = @options.length if new_index > @options.length
end

@sprites["option"].set_index(new_index)
@sprites["option"].refresh
...
# Set the values of each option one final time.
@options.length.times do |i|
  option = @options[i]

  # Don't accidentally call an action option during scene shutdown.
  next if option.is_a?(EnumOption) && option.values.length <= 1
...
# Legacy command-menu implementation

#

# Kept intact for compatibility with anything that still calls pbStartScene2.
...
cmdwindow.height =
  Graphics.height - sprites["textbox"].height

cmdwindow.viewport = viewport
cmdwindow.visible = true

sprites["textbox"].text =
  commands.getDesc(cmdwindow.index)
...
def pbHardcoreMode
  ...
  if pbConfirmMessageSerious(
       _INTL("Are you sure you want to enable 'Hardcore Mode'? This can only be reversed by starting a new save.")
     )
...
def pbNuzlockeMode
  ...
  if pbConfirmMessageSerious(
       _INTL("Are you sure you want to enable 'Nuzlocke Mode'? This can only be reversed by starting a new save.")
     )
...
def pbOneRoute
  ...
  if pbConfirmMessageSerious(
       _INTL("Are you sure you want to enable 'One Catch per Map'? This can only be reversed by starting a new save.")
     )
...
def pbDupsClause
  ...
  if pbConfirmMessageSerious(
       _INTL("Are you sure you want to disable 'Dups Clause'? This can only be reversed by starting a new save.")
     )
...
def pbStaticClause
  ...
  if pbConfirmMessageSerious(
       _INTL("Are you sure you want to disable 'Static Encounters'? This can only be reversed by starting a new save.")
     )
...
def pbShinyClause
  ...
  if pbConfirmMessageSerious(
       _INTL("Are you sure you want to disable 'Shiny Encounters'? This can only be reversed by starting a new save.")
     )
...
def pbNicknameClause
  ...
  if pbConfirmMessageSerious(
       _INTL("Are you sure you want to enable 'Forced Nicknames'? This can only be reversed by starting a new save.")
     )
...
def pbSleepClause
  ...
  if pbConfirmMessageSerious(
       _INTL("Are you sure you want to enable 'A Full Eight Hours'? This can only be reversed by starting a new save.")
     )
...
def pbOverworldClause
  ...
  if pbConfirmMessageSerious(
       _INTL("Are you sure you want to enable 'No Overworld Capture'? This can only be reversed by starting a new save.")
     )
...
def pbStatuesClause
  ...
  if pbConfirmMessageSerious(
       _INTL("Are you sure you want to enable 'No Statues'? This can only be reversed by starting a new save.")
     )
...
def pbSurvivalClause
  ...
  if pbConfirmMessageSerious(
       _INTL("Are you sure you want to enable 'Survival Mode (Pokemon)'? This can only be reversed by starting a new save.")
     )
...
MenuHandlers.add(:options_menu, :autotarget, {
  ...
  "set_proc"    => proc { |value, _scene|
    $PokemonSystem.autotarget = value
  }
})
...
MenuHandlers.add(:options_menu, :text_speed, {
  ...
  "set_proc"    => proc { |value, scene|
    ...
    MessageConfig.pbSetTextSpeed(
      MessageConfig.pbSettingToTextSpeed(value)
    )

    scene.sprites["textbox"].textspeed =
      MessageConfig.pbGetTextSpeed

    scene.sprites["textbox"].letterbyletter = true
    scene.sprites["textbox"].text =
      scene.sprites["textbox"].text
  }
})
...
MenuHandlers.add(:options_menu, :simple_title, {
  ...
  "get_proc"    => proc {
    next getSIDataStatus("classicTitleScreen")
  },
  "set_proc"    => proc { |value, _scene|
...
MenuHandlers.add(:options_menu, :disable_fogs, {
  ...
  "set_proc"    => proc { |value, _scene|
    $PokemonSystem.disable_fogs = value
  }
})
...
MenuHandlers.add(:options_menu, :battle_animations, {
  ...
  "set_proc"    => proc { |value, _scene|
    $PokemonSystem.battlescene = value
  }
})
...
MenuHandlers.add(:options_menu, :speech_frame, {
  ...
  "set_proc"    => proc { |value, scene|
    ...
    MessageConfig.pbSetSpeechFrame(
      ...
      Settings::SPEECH_WINDOWSKINS[value]
    )

    scene.sprites["textbox"].setSkin(
...
MenuHandlers.add(:options_menu, :menu_frame, {
  ...
  "set_proc"    => proc { |value, scene|
    ...
    MessageConfig.pbSetSystemFrame(
      "Graphics/Windowskins/" +
      Settings::MENU_WINDOWSKINS[value]
    )

    scene.sprites["option"].setSkin(
      MessageConfig.pbGetSystemFrame
    )
  }
})
...
MenuHandlers.add(:options_menu, :send_to_boxes, {
  ...
  "condition"   => proc {
    next Settings::NEW_CAPTURE_CAN_REPLACE_PARTY_MEMBER
  },
  "get_proc"    => proc { next $PokemonSystem.sendtoboxes },
  "set_proc"    => proc { |value, _scene|
    $PokemonSystem.sendtoboxes = value
  }
})
...
MenuHandlers.add(:options_menu, :wbattle_music, {
  ...
  "set_proc"    => proc { |value, scene|
    ...
    if testbgm == $game_temp.memorized_bgm &&
       $game_system.is_a?(Game_System)
...
MenuHandlers.add(:options_menu, :bossbattle_music, {
  ...
  "set_proc"    => proc { |value, scene|
    $PokemonSystem.bossbattleMusic = value

    if $game_system.is_a?(Game_System) && !$game_temp.memorized_bgm
      $game_system.bgm_pause
      $game_temp.memorized_bgm = $game_system.getPlayingBGM
      $game_temp.memorized_bgm_position =
        (Audio.bgm_pos rescue 0)
    end
...
MenuHandlers.add(:options_menu, :wvbattle_music, {
  ...
  "set_proc"    => proc { |value, _scene|
    $PokemonSystem.wildvictoryMusic = value
  }
})
...
MenuHandlers.add(:options_menu, :tvbattle_music, {
  ...
  "set_proc"    => proc { |value, _scene|
    $PokemonSystem.trainervictoryMusic = value
  }
})
...
MenuHandlers.add(:options_menu, :lowbattle_music, {
  ...
  "set_proc"    => proc { |value, scene|
    $PokemonSystem.lowHPMusic = value
...
MenuHandlers.add(:options_menu, :hardcore, {
  ...
  "set_proc"    => proc { |_value, scene|
    next if scene.in_load_screen

    if $PokemonGlobal.hardcore
      scene.sprites["textbox"].text =
        _INTL("'Hardcore Mode' is already enabled, and cannot be disabled.")
      next
    end
...
MenuHandlers.add(:options_menu, :permadeath, {
  ...
  "set_proc"    => proc { |_value, scene|
    ...
    if nuzlocke_has?(:PERMADEATH)
      scene.sprites["textbox"].text =
        _INTL("'Nuzlocke Mode' is already enabled, and cannot be disabled.")
      next
    end
...
MenuHandlers.add(:options_menu, :one_route, {
  ...
  "set_proc"    => proc { |_value, scene|
    ...
    if nuzlocke_has?(:ONEROUTE)
      scene.sprites["textbox"].text =
        _INTL("'One Catch per Map' is already enabled, and cannot be disabled.")
      next
    end
...
MenuHandlers.add(:options_menu, :dups_clause, {
  ...
  "set_proc"    => proc { |_value, scene|
    ...
    if !nuzlocke_has?(:DUPSCLAUSE)
      scene.sprites["textbox"].text =
        _INTL("'Dups Clause' is already disabled, and cannot be enabled.")
      next
    end
...
MenuHandlers.add(:options_menu, :static_clause, {
  ...
  "set_proc"    => proc { |_value, scene|
    ...
    if !nuzlocke_has?(:STATIC)
      scene.sprites["textbox"].text =
        _INTL("'Static Encounters' is already disabled, and cannot be enabled.")
      next
    end
...
MenuHandlers.add(:options_menu, :shiny_clause, {
  ...
  "set_proc"    => proc { |_value, scene|
    ...
    if !nuzlocke_has?(:SHINY)
      scene.sprites["textbox"].text =
        _INTL("'Shiny Encounters' is already disabled, and cannot be enabled.")
      next
    end
...
MenuHandlers.add(:options_menu, :nicknameforcing, {
  ...
  "set_proc"    => proc { |_value, scene|
    ...
    if nuzlocke_has?(:NICKNAMES)
      scene.sprites["textbox"].text =
        _INTL("'Forced Nicknames' is already enabled, and cannot be disabled.")
      next
    end
...
MenuHandlers.add(:options_menu, :sleepforcing, {
  ...
  "set_proc"    => proc { |_value, scene|
    ...
    if nuzlocke_has?(:AFULLEIGHTHOURS)
      scene.sprites["textbox"].text =
        _INTL("'A Full Eight Hours' is already enabled, and cannot be disabled.")
      next
    end
...
MenuHandlers.add(:options_menu, :nocatchforcing, {
  ...
  "set_proc"    => proc { |_value, scene|
    ...
    if nuzlocke_has?(:NOOVCATCHING)
      scene.sprites["textbox"].text =
        _INTL("'No Overworld Capture' is already enabled, and cannot be disabled.")
      next
    end

    if pbOverworldClause
      scene.refresh_options
    end
  }
})
...
MenuHandlers.add(:options_menu, :nostatueforcing, {
  ...
  "set_proc"    => proc { |_value, scene|
    ...
    if nuzlocke_has?(:NOSTATUES)
      scene.sprites["textbox"].text =
        _INTL("'No Statues' is already enabled, and cannot be disabled.")
      next
    end
...
MenuHandlers.add(:options_menu, :pkmnfoodandwater, {
  ...
  "set_proc"    => proc { |_value, scene|
    ...
    if nuzlocke_has?(:SURVIVALMODE)
      scene.sprites["textbox"].text =
        _INTL("'Survival Mode (Pokemon)' is already enabled, and cannot be disabled.")
      next
    end
