module PokeBattle_BattleCommon
  ...
  def pbRecordAndStoreCaughtPokemon
    @caughtPokemon.each do |pkmn|
      ...
      pbSetBattled(pkmn)   # In case the form changed upon leaving battle
  ...
  def pbThrowPokeBall(idxBattler,ball,catch_rate=nil,showPlayer=false)
    ...
    if rand(255)<10 && ($Trainer.playersleep <=50 || $Trainer.playerstamina <=15)
      pbDisplay(_INTL("The Ball Missed!"))
      return
    end
    if battler.fainted?
    ...
    when 0
      ...
      $PokemonBag.pbStoreItem(ball) if $game_variables[50]==8 && rand(100)<=20
  ...
  def pbCaptureCalc(pkmn,battler,catch_rate,ball)
    ...
    if Settings::ENABLE_CRITICAL_CAPTURES
      ...
      elsif $game_variables[50] == 12 && pbRandom(50)<10; c = x*4/12
      end
