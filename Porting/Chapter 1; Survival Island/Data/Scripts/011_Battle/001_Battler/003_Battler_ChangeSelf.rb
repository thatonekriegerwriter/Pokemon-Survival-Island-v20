class PokeBattle_Battler
  ...
  def pbRecoverHP(amt,anim=true,anyAnim=true)
    ...
    if $game_variables[50]==9 && pbOwnedByPlayer?
      ###---Healer Ability
      amt=(amt*1.5).floor
    end
    oldHP = @hp
    ...
    return amt
    if @pokemon
     pkmn.changeHappiness("vitamin",@pokemon)
     pkmn.changeLoyalty("vitamin",@pokemon)
    end
  end
  ...
  def pbReducePP(move)
    ...
    return true if $game_variables[50]==2 && @battle.pbOwnedByPlayer?(@index) && rand(5)==1
  ...
  def pbTransform(target)
    ...
    target.moves.each_with_index do |m,i|
      ...
      if @pokemon==(:DITTO) || @pokemon==(:SMEARGLE)
      @moves[i].pp       = 5
      @moves[i].total_pp = 5
      end
      end
