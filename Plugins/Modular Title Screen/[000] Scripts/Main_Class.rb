...
class ModularTitleScreen
  ...
  #-----------------------------------------------------------------------------
  # download/mystery gift notification icons (top-left corner)
  #-----------------------------------------------------------------------------
  def create_info_elements
    ...
    # nudge the mystery gift icon over if the download icon is also showing
  ...
  def create_control_elements
    ...
    # placeholder position — set_text recenters this on selectionbox before it's ever
    # made visible (visible only flips true inside set_text), so the exact value here
    # doesn't matter
    @sprites2["currentSelection"].x = @viewport.rect.width / 2
  ...
  # sets (or clears, when trainer is nil) the player charset + party icons
  ...
  def set_text(box, text)
    ...
    # currentSelection's width changes with every label ("Continue Game" vs "Options"
    # vs "Check for Updates", etc.), but its x was only ever set once at creation —
    # so its left edge stayed fixed while its width (and true center) drifted with
    # every label change, making it look off-center depending on the command shown.
    # Recentering it here, on the fixed selection-box graphic behind it, keeps it
    # visually centered no matter which label is showing.
    if box == "currentSelection"
      @sprites2[box].x = @sprites2["selectionbox"].x - (@sprites2[box].width / 2)
    end
  ...
  def create_save_data_boxes
    ...
    (1..5).each do |i|
      @sprites2["star#{i}"] = IconSprite.new(89 + 20 * (i - 1), 244, @viewport)
      @sprites2["star#{i}"].setBitmap("Graphics/Pictures/stars1.png")
      @sprites2["star#{i}"].z = 1009
      @sprites2["star#{i}"].visible = false
    end
    ...
    difficulty_offsets = {"info" => 120, "veryeasy" => 159, "easy" => 186, "normal" => 213,
                           "hard" => 240, "veryhard" => 267, "return" => 294}
    difficulty_offsets.each do |label, y|
      createtextboxesplease(label, @sprites2["choicebox"].x - 54, y)
      @sprites2[label].z = 1008
    end
  ...
  def pbChoiceBox(commands=nil)
    if commands.nil?
      ...
      @sprites2["choicebox"].bitmap = pbBitmap(commands.length > 3 ? "Graphics/MODTS/choice box2" : "Graphics/MODTS/choice box")
  ...
  def move_selector_arrow(selection)
    ...
    theoptions = [@sprites2["veryeasy"], @sprites2["easy"], @sprites2["normal"],
                  @sprites2["hard"], @sprites2["veryhard"], @sprites2["return"]]
  ...
  def displayboxdisplaymeasavefile(file=nil, header=nil, selected_file=nil)
    if file.nil? || file == 1
      ...
      (1..5).each { |i| @sprites2["star#{i}"].visible = false }
      if file.nil?
        @sprites2["selectionbox2"].visible = false
        @sprites2["arrowl2"].visible = false
        @sprites2["arrowr2"].visible = false
  ...
  # advances one frame of animation/logic (update) plus one render (Graphics.update);
  # the arrow-bump animations below call this repeatedly to step through their motion
  def advance_frame
    update
    Graphics.update
  end

  # bumps the sprite named by sprite_key by `step` pixels, runs the block in between
  # (used to swap displayed text partway through the bump), then eases back to start.
  # Replaces the old move_arrowr/move_arrowl/move_arrowr2/move_arrowl2, which were
  # identical apart from which sprite/direction they used.
  def animate_arrow(sprite_key, step)
    advance_frame
    2.times { advance_frame; @sprites2[sprite_key].x += step; advance_frame }
  ...
  def move_arrowr(box, text)
    animate_arrow("arrowr", 5) { set_text(box, text) }
  end

  def move_arrowl(box, text)
    animate_arrow("arrowl", -5) { set_text(box, text) }
  end

  def move_arrowr2(file=nil, header=nil, selected_file=nil)
    animate_arrow("arrowr2", 5) { displayboxdisplaymeasavefile(file, header, selected_file) }
  end

  def move_arrowl2(file=nil, header=nil, selected_file=nil)
    animate_arrow("arrowl2", -5) { displayboxdisplaymeasavefile(file, header, selected_file) }
  end
  def intro
  ...
  def update
    @currentFrame += 1
    self.updateElements
    # NOTE: this condition currently has an empty body — restart is never actually
    # triggered when the BGM finishes looping. Left as-is since fixing it changes
    # behavior; flagging in case a BGM-timeout restart was intended here.
  ...
  # function to restart the game when BGM times out (currently unimplemented — see update)
  def restart
  end
end
#===============================================================================
